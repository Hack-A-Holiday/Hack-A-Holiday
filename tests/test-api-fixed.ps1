...
Write-Host "`n2. Testing trip planning..." -ForegroundColor Yellow
$tripRequest = @{
    preferences = @{
...
try {
    ...
    Write-Host "[PASS] Trip planning successful" -ForegroundColor Green
    $tripId = $planResponse.data.tripId
    Write-Host "Trip ID: $tripId" -ForegroundColor Gray
    
    # Test 3: Get Trip (GET)
    if ($tripId) {
        Write-Host "`n3. Testing get trip..." -ForegroundColor Yellow
        try {
            $getResponse = Invoke-RestMethod -Uri "$ApiUrl/trips/$tripId" -Method GET
            Write-Host "[PASS] Get trip successful" -ForegroundColor Green
            Write-Host "Destination: $($getResponse.data.itinerary.destination)" -ForegroundColor Gray
        } catch {
            Write-Host "[FAIL] Get trip failed: $($_.Exception.Message)" -ForegroundColor Red
        }
    }
    
} catch {
    Write-Host "[FAIL] Trip planning failed: $($_.Exception.Message)" -ForegroundColor Red
...
Write-Host "`n4. Testing list trips (may fail without auth)..." -ForegroundColor Yellow
try {
    $listResponse = Invoke-RestMethod -Uri "$ApiUrl/trips" -Method GET -Headers @{"x-user-id" = "test-user"}
    Write-Host "[PASS] List trips successful" -ForegroundColor Green
    Write-Host "Trip count: $($listResponse.data.trips.Count)" -ForegroundColor Gray
} catch {
    Write-Host "[WARN] List trips failed (expected without proper auth): $($_.Exception.Message)" -ForegroundColor Yellow
}

# Test 5: Invalid endpoint
Write-Host "`n5. Testing invalid endpoint..." -ForegroundColor Yellow
try {
    $invalidResponse = Invoke-RestMethod -Uri "$ApiUrl/invalid-endpoint" -Method GET
    Write-Host "[FAIL] Invalid endpoint should have failed" -ForegroundColor Red
} catch {
    Write-Host "[PASS] Invalid endpoint correctly returned error" -ForegroundColor Green
}
