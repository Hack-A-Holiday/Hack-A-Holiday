...
class IntegratedAITravelAgent {
  ...
  async generateUserAnalytics(sessionId) {
    ...
    const totalInteractions = Array.isArray(context.conversationHistory)
  ...
  async getUserProfile(sessionId) {
    ...
    return {
      ...
      totalInteractions: Array.isArray(context.conversationHistory)
        ? context.conversationHistory.length
        : 0,
  ...
  extractPreferencesFromMessage(message, context) {
    ...
    if (
      /luxury|premium|5.star|upscale|fancy|high.end|lavish|indulgent/i.test(
        lowerMsg
      )
    ) {
      updates.travelStyle = "luxury";
    } else if (
      /budget|cheap|affordable|backpack|hostel|economical|frugal|save\s+money/i.test(
        lowerMsg
      )
    ) {
      updates.travelStyle = "budget";
    } else if (
      /mid.range|moderate|comfortable|3.star|4.star|standard/i.test(lowerMsg)
    ) {
    ...
    const newInterests = [];
    if (
      /culture|museum|art|history|heritage|monuments?|ancient|archaeology/i.test(
        lowerMsg
      )
    ) {
    ...
    if (
      /food|dining|culinary|restaurant|cuisine|eat|foodie|gastronomy/i.test(
        lowerMsg
      )
    ) {
    ...
    if (
      /adventure|hiking|trek|sports|active|outdoor|climbing|rafting/i.test(
        lowerMsg
      )
    ) {
    ...
    if (/airbnb|apartment|rental/i.test(lowerMsg))
      updates.accommodationType = "rental";
    ...
    if (/slow.paced|leisurely|relaxed|easy.going|no.rush/i.test(lowerMsg)) {
      ...
      /fast.paced|action.packed|busy|see.everything|maximize/i.test(lowerMsg)
    ) {
    ...
    // Crowd preference
    if (
      /avoid.crowds|less.crowded|off.the.beaten|quiet|peaceful|secluded/i.test(
        lowerMsg
      )
    ) {
    ...
    if (newAccessibility.length > 0) {
      ...
      updates.accessibilityNeeds = [
        ...new Set([...existing, ...newAccessibility]),
      ];
    }
    ...
    for (const airline of airlines) {
      if (
        new RegExp(`(prefer|like|love|want).*${airline}`, "i").test(message)
      ) {
        preferredAirlines.push(airline);
      } else if (
        new RegExp(`(avoid|don't like|hate|not).*${airline}`, "i").test(message)
      ) {
    ...
    if (preferredAirlines.length > 0) {
      const existing =
        context.preferences.flightPreferences?.preferredAirlines || [];
      updates.flightPreferences.preferredAirlines = [
        ...new Set([...existing, ...preferredAirlines]),
      ];
    }

    if (avoidAirlines.length > 0) {
      const existing =
        context.preferences.flightPreferences?.avoidAirlines || [];
      updates.flightPreferences.avoidAirlines = [
        ...new Set([...existing, ...avoidAirlines]),
      ];
    }
    ...
    if (preferredChains.length > 0) {
      const existing =
        context.preferences.hotelPreferences?.preferredChains || [];
      updates.hotelPreferences.preferredChains = [
        ...new Set([...existing, ...preferredChains]),
      ];
    }

    // Hotel rating
    const ratingMatch = message.match(
      /(\d)\s*star|minimum.*(\d)\s*star|at least.*(\d)\s*star/i
    );
    if (ratingMatch) {
      const rating = parseInt(
        ratingMatch[1] || ratingMatch[2] || ratingMatch[3]
      );
    ...
    if (/pet.friendly|pets.allowed|bring.*pet/i.test(lowerMsg))
      amenities.push("pet-friendly");
    if (/airport.shuttle|airport.transfer/i.test(lowerMsg))
    ...
    if (/conference|meeting.room|business.center/i.test(lowerMsg))
      amenities.push("business-facilities");

    if (amenities.length > 0) {
      const existing =
        context.preferences.hotelPreferences?.preferredAmenities || [];
      updates.hotelPreferences.preferredAmenities = [
        ...new Set([...existing, ...amenities]),
      ];
    }
    ...
    const hotelBudgetMatch = message.match(
      /\$?(\d+).*per.night|per.night.*\$?(\d+)|hotel.*budget.*\$?(\d+)/i
    );
    if (hotelBudgetMatch) {
      const budget = parseInt(
        hotelBudgetMatch[1] || hotelBudgetMatch[2] || hotelBudgetMatch[3]
      );
      if (budget > 0) {
  ...
  detectDestinationIntent(message, context) {
    ...
    if (
      /restaurant|food|dining|eat|cuisine|where to eat|best places to eat|popular restaurants/i.test(
        lowerMsg
      )
    ) {
    ...
    if (
      /activity|activities|things to do|attractions|sightseeing|what to do|places to visit|popular activities|tourist spots/i.test(
        lowerMsg
      )
    ) {
    ...
    // Detect photo intent
    if (
      /photo|picture|image|show me|what.*look like|gallery|view/i.test(lowerMsg)
    ) {
  ...
  extractDestination(message, context) {
    ...
    for (const pattern of patterns) {
      ...
      if (match && match[1]) {
        ...
        if (
          !excludeWords.includes(destination.toLowerCase()) &&
          destination.length > 2
        ) {
          console.log(
            `   📍 Extracted destination from message: ${destination}`
          );
          return destination;
        }
      }
    ...
    if (context.currentDestinations && context.currentDestinations.length > 0) {
      ...
      if (recentDestinations.length > 0) {
        const destination =
          recentDestinations[recentDestinations.length - 1].name;
        console.log(
          `   📍 Using recent destination from context: ${destination}`
        );
        return destination;
      }
    }
    ...
    if (context.searchHistory && context.searchHistory.length > 0) {
      ...
      if (recentSearch.destination) {
        console.log(
          `   📍 Using destination from search history: ${recentSearch.destination}`
        );
  ...
  storeDestinationInContext(sessionId, destination, additionalInfo = {}) {
    ...
    const existingIndex = context.currentDestinations.findIndex(
  ...
  async enrichDestinationInfo(destination, type = "restaurants", limit = 5) {
    try {
      ...
      let searchResults;
      if (type === "restaurants") {
        searchResults = await this.tripAdvisorService.getRestaurantsNearby(
          destination,
          limit
        );
      } else {
        searchResults = await this.tripAdvisorService.getAttractionsNearby(
          destination,
          limit
        );
      }
      ...
      console.log(
        `   ✅ Found ${searchResults.length} ${type} for ${destination}`
      );
          ...
          try {
            ...
            const [details, photos] = await Promise.all([
              this.tripAdvisorService
                ...
                .catch((err) => {
                  console.warn(
                    `   ⚠️ Failed to get details for ${location.name}:`,
                    err.message
                  );
                  return null;
                }),
              this.tripAdvisorService
                .getLocationPhotos(locationId, 5)
                .catch((err) => {
                  console.warn(
                    `   ⚠️ Failed to get photos for ${location.name}:`,
                    err.message
                  );
                  return [];
                }),
            ...
            console.error(
              `   ❌ Error enriching location ${location.name}:`,
              error.message
            );
            return location;
          }
    ...
    results.forEach((location, index) => {
      ...
      if (location.rating) {
        ...
        if (location.num_reviews || location.reviewCount) {
          response += ` (${
            location.num_reviews || location.reviewCount
          } reviews)`;
        }
        response += "\n";
      }
      ...
      if (type === "restaurants") {
        if (location.cuisine && Array.isArray(location.cuisine)) {
          const cuisineNames = location.cuisine
            .map((c) =>
        ...
        if (location.price_level || location.priceLevel) {
          response += `   💰 Price: ${
            location.price_level || location.priceLevel
          }\n`;
        }
      }
  ...
  getUserProfileSummary(context) {
    ...
    if (prefs.crowdPreference) {
      summary += `👥 Crowds: ${
        prefs.crowdPreference === "avoid-crowds"
          ? "Prefers Less Crowded"
          : "Enjoys Popular Spots"
      }\n`;
      hasPreferences = true;
    }
    ...
    if (prefs.languagePreference) {
      summary += `🗣️ Language: ${
        prefs.languagePreference === "english-speaking"
          ? "Prefers English-speaking destinations"
    ...
    if (context.tripHistory && context.tripHistory.length > 0) {
      ...
      const favDestinations = context.tripHistory
        .map((t) => t.destination)
        .filter(Boolean)
        .slice(-3);
    ...
    if (hasPreferences) {
      ...
      summary +=
        '- Reference their past trips if relevant ("You enjoyed Madrid...")\n';
  ...
  async getNearbyAttractions(destination) {
    try {
      ...
      const locationDetails = await this.getTripAdvisorLocationDetails(
        locationId
      );
  ...
  async searchNearbyAttractions(latLong) {
    try {
      ...
      if (!response.ok) {
        console.error(
          "❌ TripAdvisor nearby search API error:",
          response.status,
          response.statusText
        );
        return [];
  ...
  getMockAttractionsForDestination(destination) {
    ...
    if (
      ...
      destination.toLowerCase().includes("milan") ||
      destination.toLowerCase().includes("florence") ||
      destination.toLowerCase().includes("venice")
    ) {
      return [
        {
          id: "3436969",
          ...
          description:
            "Ancient Roman amphitheater, one of the most iconic landmarks in the world. Built in 70-80 AD, it could hold up to 80,000 spectators.",
          web_url:
            "https://www.tripadvisor.com/Attraction_Review-g187791-d192285-Reviews-Colosseum-Rome_Lazio.html",
          photo_url:
            "https://images.unsplash.com/photo-1552832230-c0197dd311b5?w=400&h=300&fit=crop&crop=center",
        },
        {
          id: "3436970",
          ...
          description:
            "Extensive collection of art and historical artifacts, including the famous Sistine Chapel with Michelangelo's ceiling frescoes.",
          web_url:
            "https://www.tripadvisor.com/Attraction_Review-g187791-d192285-Reviews-Vatican_Museums-Rome_Lazio.html",
          photo_url:
        ...
        },
        {
          ...
          description:
            "Famous Baroque fountain designed by Nicola Salvi. Legend says throwing a coin into the fountain ensures your return to Rome.",
          web_url:
            "https://www.tripadvisor.com/Attraction_Review-g187791-d192285-Reviews-Trevi_Fountain-Rome_Lazio.html",
          photo_url:
            "https://images.unsplash.com/photo-1587474260584-136574528ed5?w=400&h=300&fit=crop&crop=center",
        },
      ];
    }
      ...
      return [
        {
          ...
          description:
            "Iconic iron lattice tower and symbol of Paris. Built in 1889, it offers stunning views of the city from its observation decks.",
          web_url:
        ...
        },
        {
          ...
          description:
            "World's largest art museum and historic monument. Home to the Mona Lisa and thousands of other masterpieces.",
          web_url:
            "https://www.tripadvisor.com/Attraction_Review-g187147-d188151-Reviews-Louvre_Museum-Paris_Ile_de_France.html",
          photo_url:
            "https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=400&h=300&fit=crop&crop=center",
        },
        {
          ...
          description:
            "Medieval Catholic cathedral, masterpiece of French Gothic architecture. Famous for its flying buttresses and gargoyles.",
          web_url:
            "https://www.tripadvisor.com/Attraction_Review-g187147-d188151-Reviews-Notre_Dame_Cathedral-Paris_Ile_de_France.html",
          photo_url:
            "https://images.unsplash.com/photo-1587474260584-136574528ed5?w=400&h=300&fit=crop&crop=center",
        },
      ];
    ...
    if (
      ...
      destination.toLowerCase().includes("bombay") ||
      destination.toLowerCase().includes("bom")
    ) {
      return [
        {
          id: "3436969",
          ...
          description:
            "Historic arch monument overlooking the Arabian Sea, built to commemorate the visit of King George V and Queen Mary. This iconic landmark is Mumbai's most famous tourist attraction and a symbol of the city.",
          web_url:
            "https://www.tripadvisor.com/Attraction_Review-g304554-d319507-Reviews-Gateway_of_India-Mumbai_Maharashtra.html",
          photo_url:
            "https://images.unsplash.com/photo-1587474260584-136574528ed5?w=400&h=300&fit=crop&crop=center",
        },
        {
          ...
          description:
            "Ancient rock-cut cave temples dedicated to Lord Shiva, accessible by ferry from Gateway of India. These 6th-century caves feature intricate sculptures and are a UNESCO World Heritage Site.",
          web_url:
            "https://www.tripadvisor.com/Attraction_Review-g304554-d319508-Reviews-Elephanta_Caves-Mumbai_Maharashtra.html",
          photo_url:
            "https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=400&h=300&fit=crop&crop=center",
        },
        {
          ...
          description:
            "Formerly Prince of Wales Museum, houses extensive collection of Indian art and artifacts. This beautiful Indo-Saracenic building showcases over 50,000 artifacts spanning 5,000 years of Indian history.",
          web_url:
            "https://www.tripadvisor.com/Attraction_Review-g304554-d319509-Reviews-Chhatrapati_Shivaji_Maharaj_Vastu_Sangrahalaya-Mumbai_Maharashtra.html",
          photo_url:
            "https://images.unsplash.com/photo-1587474260584-136574528ed5?w=400&h=300&fit=crop&crop=center",
        },
      ];
      ...
      return [
        {
          ...
          description:
            "Historic fort complex, UNESCO World Heritage Site. Built by Mughal emperor Shah Jahan, this massive red sandstone fort is a symbol of Delhi.",
          web_url:
            "https://www.tripadvisor.com/Attraction_Review-g304551-d319505-Reviews-Red_Fort-Delhi.html",
          photo_url:
            "https://images.unsplash.com/photo-1587474260584-136574528ed5?w=400&h=300&fit=crop&crop=center",
        },
        {
          ...
          description:
            "Tallest brick minaret in the world, UNESCO World Heritage Site. This 73-meter tall tower is a masterpiece of Indo-Islamic architecture.",
          web_url:
            "https://www.tripadvisor.com/Attraction_Review-g304551-d319506-Reviews-Qutub_Minar-Delhi.html",
          photo_url:
            "https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=400&h=300&fit=crop&crop=center",
        },
        {
          ...
          description:
            "War memorial arch dedicated to Indian soldiers who died in World War I. A popular gathering spot and symbol of national pride.",
          web_url:
            "https://www.tripadvisor.com/Attraction_Review-g304551-d319507-Reviews-India_Gate-Delhi.html",
          photo_url:
            "https://images.unsplash.com/photo-1587474260584-136574528ed5?w=400&h=300&fit=crop&crop=center",
        },
      ];
      ...
      return [
        {
          ...
          description:
            "Iconic white marble mausoleum, one of the Seven Wonders of the World. Built by Mughal emperor Shah Jahan in memory of his wife Mumtaz Mahal.",
          web_url:
            "https://www.tripadvisor.com/Attraction_Review-g297683-d319504-Reviews-Taj_Mahal-Agra_Uttar_Pradesh.html",
          photo_url:
        ...
        },
        {
          ...
          address: "Agra, Uttar Pradesh, India",
          description:
            "UNESCO World Heritage Site, this massive red sandstone fort was the main residence of Mughal emperors until 1638.",
          web_url:
            "https://www.tripadvisor.com/Attraction_Review-g297683-d319505-Reviews-Agra_Fort-Agra_Uttar_Pradesh.html",
          photo_url:
            "https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=400&h=300&fit=crop&crop=center",
        },
        {
          ...
          description:
            "UNESCO World Heritage Site, this abandoned Mughal city showcases the architectural genius of Emperor Akbar.",
          web_url:
            "https://www.tripadvisor.com/Attraction_Review-g297683-d319506-Reviews-Fatehpur_Sikri-Agra_Uttar_Pradesh.html",
          photo_url:
            "https://images.unsplash.com/photo-1587474260584-136574528ed5?w=400&h=300&fit=crop&crop=center",
        },
      ];
    ...
    if (
      ...
      destination.toLowerCase().includes("nyc")
    ) {
      return [
        {
          id: "3436969",
          ...
          description:
            "Iconic symbol of freedom and democracy, gifted by France to the United States. Take a ferry to visit this UNESCO World Heritage Site.",
          web_url:
            "https://www.tripadvisor.com/Attraction_Review-g60763-d319504-Reviews-Statue_of_Liberty-New_York_City_New_York.html",
          photo_url:
            "https://images.unsplash.com/photo-1524492412937-b28074a5d7da?w=400&h=300&fit=crop&crop=center",
        },
        {
          ...
          description:
            "843-acre urban park in Manhattan, featuring lakes, meadows, and walking paths. A green oasis in the heart of the city.",
          web_url:
            "https://www.tripadvisor.com/Attraction_Review-g60763-d319505-Reviews-Central_Park-New_York_City_New_York.html",
          photo_url:
            "https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=400&h=300&fit=crop&crop=center",
        },
        {
          ...
          description:
            "Art Deco skyscraper with observation decks offering panoramic views of Manhattan. An iconic symbol of New York City.",
          web_url:
            "https://www.tripadvisor.com/Attraction_Review-g60763-d319506-Reviews-Empire_State_Building-New_York_City_New_York.html",
          photo_url:
            "https://images.unsplash.com/photo-1587474260584-136574528ed5?w=400&h=300&fit=crop&crop=center",
        },
      ];
      ...
      return [
        {
          ...
          description:
            "Iconic clock tower and symbol of London. Part of the Palace of Westminster, this Gothic Revival masterpiece is a UNESCO World Heritage Site.",
          web_url:
            "https://www.tripadvisor.com/Attraction_Review-g186338-d319504-Reviews-Big_Ben-London_England.html",
          photo_url:
            "https://images.unsplash.com/photo-1524492412937-b28074a5d7da?w=400&h=300&fit=crop&crop=center",
        },
        {
          ...
          description:
            "Historic castle on the north bank of the River Thames. Home to the Crown Jewels and the famous Beefeaters.",
          web_url:
            "https://www.tripadvisor.com/Attraction_Review-g186338-d319505-Reviews-Tower_of_London-London_England.html",
          photo_url:
            "https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=400&h=300&fit=crop&crop=center",
        },
        {
          ...
          description:
            "World-famous museum housing over 8 million works, including the Rosetta Stone and Elgin Marbles.",
          web_url:
            "https://www.tripadvisor.com/Attraction_Review-g186338-d319506-Reviews-British_Museum-London_England.html",
          photo_url:
            "https://images.unsplash.com/photo-1587474260584-136574528ed5?w=400&h=300&fit=crop&crop=center",
        },
      ];
      ...
      return [
        {
          ...
          address: "Champ de Mars, 7th arrondissement, Paris",
          description:
            "Iconic iron lattice tower and symbol of Paris. Built for the 1889 World's Fair, it offers stunning views from its observation decks.",
          web_url:
            "https://www.tripadvisor.com/Attraction_Review-g187147-d187147-Reviews-Eiffel_Tower-Paris_Ile_de_France.html",
          photo_url:
            "https://images.unsplash.com/photo-1524492412937-b28074a5d7da?w=400&h=300&fit=crop&crop=center",
        },
        {
        ...
        },
        {
          ...
          address: "6 Parvis Notre-Dame, 4th arrondissement, Paris",
          description:
            "Medieval Catholic cathedral, masterpiece of French Gothic architecture. Currently under restoration after the 2019 fire.",
          web_url:
            "https://www.tripadvisor.com/Attraction_Review-g187147-d188757-Reviews-Notre_Dame_Cathedral-Paris_Ile_de_France.html",
          photo_url:
            "https://images.unsplash.com/photo-1587474260584-136574528ed5?w=400&h=300&fit=crop&crop=center",
        },
      ];
      ...
      return [
        {
          ...
          description:
            "Tokyo's oldest temple, founded in 628 AD. A vibrant Buddhist temple complex in the historic Asakusa district.",
          web_url:
            "https://www.tripadvisor.com/Attraction_Review-g298184-d319504-Reviews-Senso_ji_Temple-Tokyo_Tokyo_Prefecture.html",
          photo_url:
            "https://images.unsplash.com/photo-1524492412937-b28074a5d7da?w=400&h=300&fit=crop&crop=center",
        },
        {
          ...
          description:
            "634-meter tall broadcasting tower and observation deck. Offers panoramic views of Tokyo and Mount Fuji on clear days.",
          web_url:
            "https://www.tripadvisor.com/Attraction_Review-g298184-d319505-Reviews-Tokyo_Skytree-Tokyo_Tokyo_Prefecture.html",
          photo_url:
            "https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=400&h=300&fit=crop&crop=center",
        },
        {
          ...
          description:
            "Shinto shrine dedicated to Emperor Meiji and Empress Shoken. Surrounded by a peaceful forest in the heart of Tokyo.",
          web_url:
            "https://www.tripadvisor.com/Attraction_Review-g298184-d319506-Reviews-Meiji_Shrine-Tokyo_Tokyo_Prefecture.html",
          photo_url:
            "https://images.unsplash.com/photo-1587474260584-136574528ed5?w=400&h=300&fit=crop&crop=center",
        },
      ];
    ...
    return [
      ...
      },
      {
        ...
        address: "6 Parvis Notre-Dame, 4th arrondissement, Paris",
        description:
          "Medieval Catholic cathedral, masterpiece of French Gothic architecture.",
        web_url:
          "https://www.tripadvisor.com/Attraction_Review-g187147-d188757-Reviews-Notre_Dame_Cathedral-Paris_Ile_de_France.html",
        photo_url: null,
      },
   ...
   */
  async generateMultiMessageResponse(
    bedrockResponse,
    realData,
    tripAdvisorData,
    queryIntent,
    ...
    if (hotelResults && hotelResults.results && hotelResults.results.length > 0) {
      ...
      const guests = queryIntent.extractedInfo?.guests || 
        (typeof queryIntent.extractedInfo?.passengers === 'object' 
          ? (queryIntent.extractedInfo.passengers.adults + (queryIntent.extractedInfo.passengers.children || 0))
          : queryIntent.extractedInfo?.passengers) || 2;
  ...
  async processMessage(messageData) {
    try {
      ...
      const effectiveUserId =
        userId || userContext.userId || userContext.email || "anonymous";
      ...
      const extractedPrefs = this.extractPreferencesFromMessage(
        userQuery,
        userContextData
      );
      ...
      if (Object.keys(extractedPrefs).length > 0) {
        ...
        console.log(
          "   ✨ Extracted new preferences from message:",
          extractedPrefs
        );
      }
      ...
      if (userContextData.preferences) {
        userPreferences = {
          ...
          travelStyle:
            userContextData.preferences.travelStyle ||
      ...
      if (userContext.preferences) {
        console.log(
          "   📋 Merging trip preferences from request:",
        ...
        userPreferences = {
          ...
          travelStyle:
            userContext.preferences.travelStyle || userPreferences.travelStyle,
          interests:
            userContext.preferences.interests || userPreferences.interests,
          travelers:
            userContext.preferences.travelers || userPreferences.travelers,
          duration:
            userContext.preferences.duration || userPreferences.duration,
      ...
      const queryIntent = await this.analyzeQueryIntent(
        userQuery,
        userContext,
        conversationHistory,
        effectiveSessionId
      ...
      if (
        queryIntent.multiDestination &&
        queryIntent.extractedInfo.destinations
      ) {
        ...
        for (const destination of queryIntent.extractedInfo.destinations) {
          ...
          const flightInfo = {
            origin:
              queryIntent.extractedInfo.origin ||
              userContext?.preferences?.homeCity,
            destination: destination,
            departureDate: queryIntent.extractedInfo.departureDate,
            returnDate: queryIntent.extractedInfo.returnDate,
            passengers:
              typeof queryIntent.extractedInfo.passengers === "object"
                ? queryIntent.extractedInfo.passengers
                : queryIntent.extractedInfo.passengers
                ? {
                    adults: queryIntent.extractedInfo.passengers,
                    children: 0,
                    infants: 0,
                  }
                : { adults: 1, children: 0, infants: 0 }, // Default to 1 adult for API calls
          };
          ...
          if (
            ...
            multiFlightData.push({
              ...
              cheapestPrice: Math.min(
                ...flightData.results.map((f) => f.price || Infinity)
              ),
              flightData,
            });
          }
        ...
        if (multiFlightData.length > 0) {
          ...
          console.log(
            `   💰 DEBUG: First flight data currency: ${multiFlightData[0]?.flightData?.currency}`
          );
          ...
          console.log(
            `   ✅ Multi-destination data fetched (${currency}):`,
            multiFlightData
              .map((d) => `${d.destination}: ${d.cheapestPrice}`)
              .join(", ")
          );
        }
      }
      // For trip planning, fetch both flights and hotels
        ...
        if (queryIntent.needsHotelData) {
          ...
          console.log(
            "   🏨 DEBUG: Hotel info before date calculation:",
            JSON.stringify(hotelInfo, null, 2)
          );
          if (
            (hotelInfo.departureDate || hotelInfo.returnDate) &&
            !hotelInfo.checkIn
          ) {
            ...
            console.log(
              `   🏨 Calculated hotel dates: Check-in ${hotelInfo.checkIn}, Check-out ${hotelInfo.checkOut}`
            );
          } else {
            console.log(
              "   🏨 DEBUG: No hotel dates calculated - missing departureDate/returnDate or checkIn already set"
            );
          }
          const hotelData = await this.fetchHotelData(
            hotelInfo,
            userPreferences
          );
          console.log(
            "🔍 DEBUG: hotelData after fetch:",
            hotelData ? "EXISTS" : "NULL"
          );
          console.log("🔍 DEBUG: hotelData type:", hotelData?.type);
          console.log(
            "🔍 DEBUG: hotelData results length:",
          ...
          if (hotelData) {
            if (flightData) {
              ...
              console.log(
                "🔍 DEBUG: Created combined realData with hotels and flights"
              );
            } else {
              realData = hotelData;
        ...
        const destination =
          queryIntent.extractedInfo?.destination?.toLowerCase() || "";
        const destinations =
          queryIntent.extractedInfo?.destinations?.map((d) =>
            d.toLowerCase()
          ) || [];
        ...
        if (countryDestinations.length > 0) {
          console.log(
            `   ⚠️ Country destination detected: ${countryDestinations.join(
              ", "
            )} - asking for specific city`
          );
        ...
        if (
          ...
          for (const destination of queryIntent.extractedInfo.destinations) {
            ...
            if (
              hotelData &&
              hotelData.results &&
              hotelData.results.length > 0
            ) {
              multiHotelData.push({
                destination,
                cheapestPrice: Math.min(
                  ...hotelData.results.map((h) => h.price || Infinity)
                ),
                averagePrice: Math.round(
                  hotelData.results.reduce(
                    (sum, h) => sum + (h.price || 0),
                    0
                  ) / hotelData.results.length
                ),
          ...
          if (multiHotelData.length > 0) {
            ...
            console.log(
              "   ✅ Multi-destination hotel data fetched:",
              multiHotelData
                .map((d) => `${d.destination}: $${d.cheapestPrice}/night`)
                .join(", ")
            );
          }
        } else {
      ...
      if (queryIntent.needsHotelData) {
        const destinationForPOI =
          queryIntent.extractedInfo?.destination ||
          userPreferences?.currentTripDestination ||
          (Array.isArray(userPreferences?.preferredDestinations) &&
            userPreferences.preferredDestinations[0]);
        if (destinationForPOI) {
          try {
            tripAdvisorData = await this.tripAdvisorService.getTravelDataForAI(
              destinationForPOI,
              {
                interests: userPreferences?.interests,
                cuisine:
                  userPreferences?.hotelPreferences?.preferredAmenities?.includes(
                    "restaurant"
                  )
                    ? "local"
                    : null,
                amenities:
                  userPreferences?.hotelPreferences?.preferredAmenities,
        ...
        const requiredButtons = realData.destinations
          .map((dest) => {
            ...
            if (dest.flightData?.request) {
              ...
              const origin =
                realData.origin || queryIntent.extractedInfo?.origin || "";
              const destination = dest.destination || dest.city || "";
              const departureDate =
                realData.departureDate ||
                queryIntent.extractedInfo?.departureDate ||
                "";
              const returnDate =
                realData.returnDate ||
                queryIntent.extractedInfo?.returnDate ||
                "";
            ...
            return googleUrl
              ? `[GOOGLE_FLIGHTS_BUTTON]${googleUrl}[/GOOGLE_FLIGHTS_BUTTON]`
              : null;
          })
          .filter(Boolean);
        // Always append all required buttons if not present
        const missingButtons = requiredButtons.filter(
          (btn) => !bedrockResponse.includes(btn)
        );
        if (missingButtons.length > 0) {
          bedrockResponse =
            bedrockResponse.trim() + "\n" + missingButtons.join("\n") + "\n";
          console.log(
            `   ✅ Appended ${missingButtons.length} missing Google Flights button markers to response (multi-destination fallback).`
          );
        }
      }
      ...
      if (
        queryIntent.type === "trip_planning" &&
        realData &&
        queryIntent.extractedInfo?.destination
      ) {
        this.updateUserContext(effectiveSessionId, {
      ...
      if (queryIntent.type === "flight_search" && realData) {
        ...
        if (
          (realData.type === "flight" && realData.totalResults === 0) ||
          ...
          if (
            ...
            let googleFlightsUrl = "https://www.google.com/travel/flights";
            if (retDate) {
              googleFlightsUrl += `?q=Flights%20from%20${encodeURIComponent(
                origin
              )}%20to%20${encodeURIComponent(
                destination
              )}%20on%20${depDate}%20returning%20${retDate}`;
            } else if (depDate) {
              googleFlightsUrl += `?q=Flights%20from%20${encodeURIComponent(
                origin
              )}%20to%20${encodeURIComponent(destination)}%20on%20${depDate}`;
            } else {
              googleFlightsUrl += `?q=Flights%20from%20${encodeURIComponent(
                origin
    ...
    try {
      ...
      const conversationText = conversationHistory
        .map((turn, i) => {
          ...
          return `Turn ${i + 1}:\nUser: ${userMsg}\nAssistant: ${
            assistantMsg?.slice(0, 300) || "No response"
          }`;
        })
        .join("\n\n");
...
Summary (2-3 sentences, only explicit information):`;
      ...
      console.error(
        "   ⚠️ Error creating conversation summary:",
        error.message
      );
      return null;
    }
...
- "Plan a complete trip" = trip_planning (API needed for both flights and hotels)
- "Plan a trip from X to Y" = trip_planning (API needed for both flights and hotels)
- "Create an itinerary" = trip_planning (API needed for both flights and hotels)
- "Help me plan my trip" = trip_planning (API needed for both flights and hotels)
- If query mentions "complete trip", "full itinerary", "trip planning", "plan my trip" = trip_planning
...
- **TRIP PLANNING RULES**: If intent is trip_planning, ALWAYS set both needsFlightAPI: true AND needsHotelAPI: true (trip planning needs both flights and hotels)
...
JSON:`;
      ...
      try {
        ...
        console.log("   🧠 Nova Lite Analysis:", {
          ...
          needsAPIs: {
            flight: analysis.needsFlightAPI,
            hotel: analysis.needsHotelAPI,
          },
        ...
        if (
          analysis.extractedDestinations &&
          analysis.extractedDestinations.length === 1 &&
          (analysis.reasoning?.includes("multiple") ||
            analysis.reasoning?.includes("three") ||
            analysis.reasoning?.includes("comparison"))
        ) {
          console.log(
            "   ⚠️ WARNING: Nova Lite reasoning mentions multiple destinations but only extracted 1!"
          );
        }
  ...
  async analyzeQueryIntent(
    ...
    const contextForAnalysis = {
      ...
      fullConversationHistory: conversationHistory.map((msg) => ({
        user: msg.user || msg.content || "",
        assistant:
          typeof (msg.assistant || msg.content || "") === "string"
            ? (msg.assistant || msg.content || "").slice(0, 200)
            : "",
      })),
    };
    const novaAnalysis = await this.analyzeQueryWithNovaLite(
      query,
      contextForAnalysis
    );
    ...
    if (
      ...
      (novaAnalysis.needsFlightAPI || novaAnalysis.needsHotelAPI)
    ) {
    ...
    if (conversationSummary && extractedDestinations.length === 0) {
      ...
      const summaryDestinations = await this.extractDestinationsWithBedrock(
        conversationSummary
      );
      if (summaryDestinations && summaryDestinations.length > 0) {
        extractedDestinations = summaryDestinations;
        console.log(
          "   ✅ Found destinations in conversation summary:",
          extractedDestinations
        );
      }
    }
    ...
    if (
      ...
      if (previousFlightQuery) {
        console.log(
          "   📍 Found previous flight query:",
          previousFlightQuery.user
        );
    ...
    if (novaAnalysis.isFollowUpAnswer) {
      ...
      if (
        novaAnalysis.extractedOrigin &&
        lastAssistant.includes("where are you flying from")
      ) {
        console.log(
          "   📍 User provided origin city:",
          novaAnalysis.extractedOrigin
        );
        ...
        if (!context.preferences.homeCity) {
          ...
          console.log(
            `   🏠 Saved ${novaAnalysis.extractedOrigin} as home city for future searches`
          );
        }

        console.log(
          "   ✈️ Detected follow-up: origin provided for flight search"
        );
        console.log(
        ...
        for (let i = conversationHistory.length - 1; i >= 0; i--) {
          ...
          if (turn && turn.user) {
            console.log(
              `   🔍 Checking conversation turn ${i} for destinations:`,
              turn.user
            );
            const prevDestinations = await this.extractDestinationsWithBedrock(
              turn.user
            );
            ...
            if (prevDestinations && prevDestinations.length > 0) {
              ...
              console.log(
                `   🔎 After filtering regional names:`,
                validDestinations
              );
              ...
              if (validDestinations.length > 0) {
                ...
                console.log(
                  "   ✅ Recovered valid destinations from previous turn:",
                  validDestinations
                );
                console.log(
                  "   ✅ extractedDestinations array now contains:",
                  extractedDestinations
                );
                break;
              } else if (prevDestinations.length > 0) {
                console.log(
                  "   ⚠️ Found regional destinations, continuing search:",
                  prevDestinations
                );
              }
            }
        ...
        console.log(
          "   🎯 Final extractedDestinations after recovery:",
          extractedDestinations
        );
      }
      ...
      // Case 2: User confirming to use home city
        ...
        if (context.preferences.homeCity) {
          console.log(
            `   🏠 User confirmed using home city: ${context.preferences.homeCity}`
          );
          ...
          for (let i = conversationHistory.length - 1; i >= 0; i--) {
            ...
            if (turn && turn.user) {
              const prevDestinations =
                await this.extractDestinationsWithBedrock(turn.user);
              if (prevDestinations && prevDestinations.length > 0) {
                extractedDestinations = prevDestinations;
                console.log(
                  "   ✅ Recovered destination from previous query:",
                  extractedDestinations
                );
                break;
              }
      ...
      // Case 3: User providing destination after being asked to specify city (e.g., "Tokyo" after "Japan" clarification)
        ...
        console.log(
          "   ✈️ User provided specific city after country clarification:",
          extractedDestinations[0]
        );
        intent.extractedInfo.destination = extractedDestinations[0];
        intent.needsFlightData = true;
        intent.type = "flight_search";
        console.log(
          "   ✈️ Detected follow-up: city specified for flight search"
        );
            ...
            if (turn.user.toLowerCase().includes(" from ")) {
              ...
              if (fromMatch) {
                ...
                if (!/\d/.test(possibleOrigin) && possibleOrigin.length > 2) {
                  ...
                  foundOrigin = true;
                  console.log(
                    `   📍 Recovered origin from conversation: ${possibleOrigin}`
                  );
                  break;
                }
              }
        ...
        if (!foundOrigin) {
          ...
          if (context.preferences.homeCity) {
            ...
            console.log(
              `   🏠 Home city available (${context.preferences.homeCity}) but not auto-using - will ask user`
            );
          } else {
      ...
      // Case 4: User providing destination for hotel search
        ...
        console.log(
          "   🏨 User provided destination for hotel search:",
          extractedDestinations[0]
        );
        intent.extractedInfo.destination = extractedDestinations[0];
        ...
        console.log(
          "   🏨 Detected follow-up: destination provided for hotel search"
        );
      }
      ...
      // Case 5: User providing origin when asked "where are you flying from"
        ...
        console.log(
          "   📍 User provided origin in response to question:",
          novaAnalysis.extractedOrigin
        );
        intent.extractedInfo.origin = novaAnalysis.extractedOrigin;
        intent.needsFlightData = true;
        ...
        console.log(
          "   🔍 Looking for destinations in previous conversation..."
        );
        for (let i = conversationHistory.length - 1; i >= 0; i--) {
          const turn = conversationHistory[i];
          if (turn && turn.user && i < conversationHistory.length - 1) {
            // Don't check current query
            const prevDestinations = await this.extractDestinationsWithBedrock(
              turn.user
            );
            if (prevDestinations && prevDestinations.length > 0) {
              extractedDestinations = prevDestinations;
              console.log(
                "   ✅ Recovered destinations from previous query:",
                extractedDestinations
              );
              break;
            }
    ...
    intent.needsFlightData =
      novaAnalysis.needsFlightAPI || intent.needsFlightData;
    ...
    if (intent.type !== "flight_search" && intent.type !== "hotel_search") {
      ...
      console.log(
        `   ✅ Preserving intent type from follow-up detection: ${intent.type}`
      );
    }
    ...
    if (novaAnalysis.extractedDepartureDate) {
      ...
      console.log(
        "   📅 Nova Lite extracted departure date:",
        infoFromNova.departureDate
      );
    }
    if (novaAnalysis.extractedReturnDate) {
      infoFromNova.returnDate = novaAnalysis.extractedReturnDate;
      console.log(
        "   📅 Nova Lite extracted return date:",
        infoFromNova.returnDate
      );
    }
    ...
    if (
      ...
      const extractedFlightInfo = this.extractFlightInfo(
        query,
        userContext,
        extractedDestinations
      );
      // Smart merge: Nova data + pattern-based + already set info (non-null values take priority)
      intent.extractedInfo = smartMerge(
        infoFromNova,
        extractedFlightInfo,
        intent.extractedInfo
      );
      console.log(
        "   ✈️ Flight search with API call, extracted info:",
        intent.extractedInfo
      );
    } else if (
      intent.type === "hotel_search" &&
      (novaAnalysis.needsHotelAPI || intent.needsHotelData)
    ) {
      const hotelInfo = await this.extractHotelInfo(
        query,
        extractedDestinations
      );
      ...
      console.log(
        "   🏨 Hotel search with API call, extracted info:",
        intent.extractedInfo
      );
      ...
      console.log(
        "   🗺️ Trip planning detected, extracted info:",
        intent.extractedInfo
      );
    } else if (intent.type === "destination_recommendation") {
      intent.extractedInfo = {
        ...intent.extractedInfo,
        ...this.extractDestinationInfo(query, userContext),
      };
      console.log(
        "   🌍 Destination recommendation, extracted info:",
        intent.extractedInfo
      );
    } else if (intent.type === "budget_inquiry") {
      intent.extractedInfo = {
        ...intent.extractedInfo,
        ...this.extractBudgetInfo(query, userContext),
      };
      console.log(
        "   💰 Budget inquiry, extracted info:",
        intent.extractedInfo
      );
    } else if (intent.type === "public_transport") {
      intent.extractedInfo = {
        ...intent.extractedInfo,
        ...this.extractTransportInfo(query, userContext),
      };
      console.log(
        "   🚇 Public transport query, extracted info:",
        intent.extractedInfo
      );
    } else {
      console.log(
        `   💬 General query (${novaAnalysis.queryType}) - No API calls needed`
      );
  ...
  extractFlightInfo(query, userContext = null, extractedDestinations = []) {
    ...
    if (extractedDestinations && extractedDestinations.length > 0) {
      ...
      console.log(
        "   ✅ Using Nova Lite extracted destination:",
        info.destination
      );
    }
    ...
    if (userContext?.preferences?.homeCity) {
      ...
      console.log(
        "   🏠 Found home city from user preferences:",
        contextOrigin
      );
    }
    ...
    if (!info.destination) {
      ...
      let fromToMatch = query.match(
        /from\s+([a-zA-Z\s]+?)\s+to\s+([a-zA-Z\s]+?)(?:\s+(?:during|in|on|for|by|at|\.|,|;))/i
      );
      if (!fromToMatch) {
        // Try simpler pattern if the above doesn't match
        fromToMatch = query.match(
          /from\s+([a-zA-Z\s]{3,20}?)\s+to\s+([a-zA-Z\s]{3,20}?)(?:\s|$)/i
        );
      }
      ...
      if (fromToMatch) {
        ...
        info.origin = extractedOrigin
          .replace(
            /\s+(arrive|reach|going|flying|travel|trip|during|in).*$/i,
            ""
          )
          .trim();
        info.destination = extractedDest
          .replace(/\s+(during|in|on|for|by|at|from).*$/i, "")
          .trim();

        console.log('   ✅ Extracted from "from X to Y":', {
          origin: info.origin,
    ...
    if (!info.destination) {
      const toMatch = query.match(
        /\bto\s+([a-zA-Z\s]{3,20}?)(?:\s+(?:during|in|on|from|for|around|,|\.|\?))/i
      );
      if (toMatch) {
        info.destination = toMatch[1].trim();
        console.log(
          '   ✅ Extracted destination from "to X":',
          info.destination
        );
      }
    }
    ...
    if (!info.origin) {
      const travelingFromMatch = query.match(
        /traveling\s+from[:\s]+([^\n\r]+?)(?=\n|- |$)/i
      );
      if (travelingFromMatch) {
        info.origin = travelingFromMatch[1].trim();
        console.log(
          '   ✅ Extracted origin from "Traveling from":',
          info.origin
        );
      }
    }
    ...
    if (!info.destination) {
      ...
      if (destMatch) {
        ...
        console.log(
          '   ✅ Extracted destination from "Destination:":',
          info.destination
        );
      }
    }
    ...
    const dateRangeMatch = query.match(
      /(jan|january|feb|february|mar|march|apr|april|may|jun|june|jul|july|aug|august|sep|sept|september|oct|october|nov|november|dec|december)\s+(\d{1,2})(?:\s+to\s+|\s*-\s*)(jan|january|feb|february|mar|march|apr|april|may|jun|june|jul|july|aug|august|sep|sept|september|oct|october|nov|november|dec|december)?\s*(\d{1,2})/i
    );
    ...
    if (dateRangeMatch) {
      ...
      const endMonth = dateRangeMatch[3]
        ? dateRangeMatch[3].toLowerCase()
        : startMonth; // Same month if not specified
      ...
      if (startMonthNum && endMonthNum) {
        info.departureDate = `${year}-${startMonthNum}-${String(
          startDay
        ).padStart(2, "0")}`;
        info.returnDate = `${year}-${endMonthNum}-${String(endDay).padStart(
          2,
          "0"
        )}`;
        console.log(
          `   📅 Extracted date range: ${info.departureDate} to ${info.returnDate}`
        );
      }
    }
    ...
    // Extract passenger count - enhanced patterns and convert to proper format
    let passengerMatch = query.match(
      /(\d+)\s+(?:passenger|people|person|traveler|adult|pax)/i
    );
    if (!passengerMatch) {
      // Try patterns like "2 adults", "1 adult 1 child", "family of 4"
      passengerMatch =
        query.match(/(\d+)\s+adult/i) ||
        query.match(/family\s+of\s+(\d+)/i) ||
        query.match(/(\d+)\s+ticket/i) ||
        query.match(/for\s+(\d+)/i);
    }
    if (passengerMatch) {
      const passengerCount = parseInt(passengerMatch[1]);
      // Convert to proper passenger object format
      info.passengers = {
        adults: passengerCount,
        children: 0,
        infants: 0,
  ...
  async extractHotelInfo(query, extractedDestinations = []) {
    ...
    if (extractedDestinations && extractedDestinations.length > 0) {
      ...
      const bedrocExtractedDest = await this.extractDestinationsWithBedrock(
        query
      );
      if (bedrocExtractedDest && bedrocExtractedDest.length > 0) {
        info.destination = bedrocExtractedDest[0];
        console.log(
          "   🏨 Extracted hotel destination via Bedrock fallback:",
          info.destination
        );
      }
    }
  ...
  extractDestinationInfo(query, userContext = null) {
    ...
    const budgetPatterns = [
      {
        pattern: /budget\s+(?:of\s+)?[\$₹€£]?(\d+(?:,\d+)*(?:k)?)/i,
        multiplier: 1,
      },
      { pattern: /under\s+[\$₹€£]?(\d+(?:,\d+)*(?:k)?)/i, multiplier: 1 },
      {
        pattern: /(\d+(?:,\d+)*(?:k)?)\s+(?:dollar|rupee|euro|pound)/i,
        multiplier: 1,
      },
    ];
    ...
    if (userContext?.preferences) {
      info.preferences.budget =
        info.preferences.budget || userContext.preferences.budget;
      info.preferences.travelStyle =
        info.preferences.travelStyle || userContext.preferences.travelStyle;
      if (
        info.preferences.interests.length === 0 &&
        userContext.preferences.interests
      ) {
  ...
  extractBudgetInfo(query, userContext = null) {
    ...
    if (
      lowerQuery.includes("without flight") ||
      lowerQuery.includes("excluding flight") ||
    ...
    if (userContext?.preferences) {
      info.travelStyle =
        info.travelStyle || userContext.preferences.travelStyle;
    }
...
Return ONLY the JSON array:`;
      ...
      try {
        ...
        if (Array.isArray(destinations)) {
          return destinations.filter(
            (dest) => dest && typeof dest === "string"
          );
        }
      } catch (parseError) {
        console.log(
          "   ⚠️ Failed to parse Bedrock extraction response as JSON:",
          extractedText
        );
  ...
  async fetchFlightData(extractedInfo, userPreferences) {
    try {
      console.log("\n🛫 ===== GOOGLE FLIGHTS URL GENERATION START =====");
      console.log(
      ...
      const origin =
        ...
        userPreferences.homeCity ||
        userPreferences.lastOrigin;

      if (!origin || !extractedInfo.destination) {
        console.log("   ⚠️ SKIPPING: Missing origin or destination");
        console.log(
          "🛫 ===== GOOGLE FLIGHTS URL GENERATION END (MISSING INFO) =====\n"
        );
        return null;
      }

      // Check if passengers count is missing - ask user for it
      const hasValidPassengers =
        extractedInfo.passengers &&
        ((typeof extractedInfo.passengers === "object" &&
          extractedInfo.passengers.adults &&
          extractedInfo.passengers.adults >= 1) ||
          (typeof extractedInfo.passengers === "number" &&
            extractedInfo.passengers >= 1));

      if (!hasValidPassengers) {
        console.log(
          "   ⚠️ MISSING: Number of passengers not specified or invalid"
        );
        console.log(
          "   📊 Current passengers value:",
          extractedInfo.passengers
        );
        console.log("   🔍 Passengers type:", typeof extractedInfo.passengers);
        console.log(
          "🛫 ===== GOOGLE FLIGHTS URL GENERATION END (NO PASSENGERS) =====\n"
        );
        return {
          ...
          returnDate: extractedInfo.returnDate,
          needsPassengerCount: true,
        };
      }

      // Convert passengers to proper format if needed
      let passengers = extractedInfo.passengers;
      ...
      // Generate Google Flights URL with passenger information
      ...
      console.log("   ✅ Returning Google Flights URL only");
      console.log(
        "🛫 ===== GOOGLE FLIGHTS URL GENERATION END (SUCCESS) =====\n"
      );

      return returnData;
    } catch (error) {
      console.error("\n   ❌ GOOGLE FLIGHTS URL GENERATION ERROR:");
      console.error("   Error Message:", error.message);
      console.log("🛫 ===== GOOGLE FLIGHTS URL GENERATION END (ERROR) =====\n");
      return null;
    }
  }

  /**
   * Build Google Flights URL for fallback
   */
  ...
  generateMockHotelsForDestination(destination) {
      ...
      return [
        {
          ...
          description:
            "Luxury hotel in the heart of Rome with stunning city views",
        ...
        },
        {
          ...
          description:
            "Modern art hotel with contemporary design and excellent location",
        ...
        },
        {
          ...
          description:
            "Elegant hotel with rooftop terrace and panoramic city views",
      ...
      return [
        {
          ...
          amenities: ["WiFi", "Pool", "Restaurant", "Spa"],
          description:
            "Iconic luxury hotel on Avenue Montaigne with Eiffel Tower views",
        ...
        },
        {
          ...
          amenities: ["WiFi", "Pool", "Restaurant", "Spa", "Rooftop Bar"],
          description:
            "Boutique hotel with modern design in the Grands Boulevards district",
    ...
    if (
      dest.includes("spain") ||
      dest.includes("madrid") ||
      dest.includes("barcelona")
    ) {
      return [
        {
        ...
        },
        {
          ...
          amenities: ["WiFi", "Pool", "Restaurant", "Fitness Center"],
          description:
            "Modernista hotel with stunning architecture and city views",
        ...
        },
        {
          ...
          amenities: ["WiFi", "Pool", "Restaurant", "Spa", "Rooftop Bar"],
          description:
            "Elegant hotel near Retiro Park with beautiful garden views",
      ...
      return [
        {
          ...
          amenities: ["WiFi", "Pool", "Restaurant", "Spa"],
          description:
            "Luxury hotel with panoramic city views and traditional Japanese service",
        ...
        },
        {
          ...
          amenities: ["WiFi", "Pool", "Restaurant", "Fitness Center"],
          description:
            "Classic luxury hotel with traditional Japanese architecture",
    ...
    return [
      {
        ...
        amenities: ["WiFi", "Pool", "Restaurant", "Spa"],
        description:
          "Modern luxury hotel with excellent city views and amenities",
  ...
  async fetchHotelData(extractedInfo, userPreferences) {
    try {
      ...
      if (!extractedInfo.checkIn || !extractedInfo.checkOut) {
        console.log(
          "   ⚠️ SKIPPING: No check-in/check-out dates found in query"
        );
      ...
      const results = await this.hotelService.searchHotelsEnhanced(
        searchRequest
      );
...
`;
    ...
    if (conversationHistory.length > 0) {
      ...
      conversationHistory.slice(-5).forEach((turn) => {
        try {
          ...
          if (assistantMsg && typeof assistantMsg === "string") {
            contextPrompt += `Assistant: ${assistantMsg.substring(0, 200)}${
              assistantMsg.length > 200 ? "..." : ""
            }\n\n`;
          }
        } catch (error) {
      ...
      if (
        ...
        contextPrompt += `\n🔍 CRITICAL CONTEXT: User asked about ${
          queryIntent.extractedInfo.destinations.length
        } destinations: ${queryIntent.extractedInfo.destinations.join(", ")}\n`;
    ...
    if (realData && !realData.error) {
      ...
      if (realData.type === "combined") {
        ...
        if (
          realData.flights &&
          realData.flights.results &&
          realData.flights.results.length > 0
        ) {
          ...
          realData.flights.results.forEach((flight, idx) => {
            ...
            contextPrompt += `- Price: ${flight.price || "N/A"} ${
              flight.currency || "USD"
            }\n`;
            contextPrompt += `- Route: ${
              flight.origin || realData.flights.request.origin
            } → ${
              flight.destination || realData.flights.request.destination
            }\n`;
            contextPrompt += `- Departure: ${
              flight.departureDate || "N/A"
            } at ${flight.departureTime || "N/A"}\n`;
            ...
            if (flight.stopDetails && flight.stopDetails.length > 0) {
              contextPrompt += `- Layovers: ${flight.stopDetails
                .map((s) => `${s.airport || s.city} (${s.duration})`)
                .join(", ")}\n`;
            }
          });
        }
        ...
        if (
          realData.hotels &&
          realData.hotels.results &&
          realData.hotels.results.length > 0
        ) {
          ...
          realData.hotels.results.slice(0, 5).forEach((hotel, idx) => {
            ...
            contextPrompt += `- Price: ${hotel.price || "N/A"} ${
              hotel.currency || "USD"
            } per night\n`;
      ...
      // Handle flight-only data (legacy - redirect to Google Flights)
      else if (realData.type === "flight") {
        contextPrompt += `IMPORTANT: Do not display Kiwi flight results. Direct user to Google Flights instead.\n`;
        if (
          realData.googleFlightsFallback &&
          realData.googleFlightsFallback.url
        ) {
          contextPrompt += `Google Flights URL: ${realData.googleFlightsFallback.url}\n`;
          contextPrompt += `Say "Here are some flights for your search:" and create a Google Flights button.\n`;
        } else {
          contextPrompt += `Create a Google Flights search URL and direct user there.\n`;
        }
        contextPrompt += `Do NOT show specific flight results, prices, or airline names.\n\n`;
      }
      // Handle missing passengers case - ask user for passenger count before Google Flights
      else if (realData.type === "missing_passengers") {
        console.log(
          "   🚨 PASSENGER VALIDATION TRIGGERED - Missing passengers detected"
        );
        contextPrompt += `Flight Search Request (INCOMPLETE - Missing Passenger Information):\n`;
        contextPrompt += `- Origin: ${realData.origin}\n`;
        contextPrompt += `- Destination: ${realData.destination}\n`;
        if (realData.departureDate) {
          contextPrompt += `- Departure: ${realData.departureDate}\n`;
        }
        if (realData.returnDate) {
          contextPrompt += `- Return: ${realData.returnDate}\n`;
        }
        contextPrompt += `- Passengers: NOT SPECIFIED (REQUIRED)\n\n`;

        contextPrompt += `RESPONSE INSTRUCTIONS:\n`;
        contextPrompt += `1. Acknowledge the flight search request\n`;
        contextPrompt += `2. Ask: "How many travelers? (e.g., '1 adult', '2 adults, 1 child', 'family of 4')"\n`;
        contextPrompt += `3. Explain that you need this information to search for flights\n`;
        contextPrompt += `4. DO NOT provide Google Flights links until passenger count is specified\n`;
        contextPrompt += `5. Keep the response friendly and helpful\n\n`;
      }
      // Handle Google Flights only data (no Kiwi flights displayed)
      else if (realData.type === "google_flights_only") {
        contextPrompt += `Flight Search Request:\n`;
        contextPrompt += `- Origin: ${realData.request.origin}\n`;
        contextPrompt += `- Destination: ${realData.request.destination}\n`;
        if (realData.request.departureDate) {
          contextPrompt += `- Departure: ${realData.request.departureDate}\n`;
        }
        if (realData.request.returnDate) {
          contextPrompt += `- Return: ${realData.request.returnDate}\n`;
        }
        contextPrompt += `- Passengers: ${realData.request.passengers}\n\n`;

        contextPrompt += `RESPONSE INSTRUCTIONS:\n`;
        contextPrompt += `1. Say "Here are some flights for your search:"\n`;
        contextPrompt += `2. Create a button linking to Google Flights: ${realData.googleFlightsUrl}\n`;
        contextPrompt += `3. Use this format: [Search Flights on Google Flights](${realData.googleFlightsUrl})\n`;
        contextPrompt += `4. Do NOT show any specific flight results, prices, or airline names\n`;
        contextPrompt += `5. Keep the response simple and direct\n`;
        contextPrompt += `6. You can mention that Google Flights will show comprehensive options\n\n`;
      }
      ...
      // Handle multi-destination flight comparison
        ...
        const currency =
          realData.currency ||
          realData.destinations[0]?.flightData?.currency ||
          "USD";
        const currencySymbol = this.getCurrencySymbol(currency);

        console.log(
          `💰 Context building - Using currency: ${currency} → ${currencySymbol}`
        );

        contextPrompt += `\n=== REAL-TIME MULTI-DESTINATION FLIGHT COMPARISON ===\n`;
        contextPrompt += `Flight prices from ${
          realData.destinations[0]?.flightData?.request?.origin || "origin"
        }:\n\n`;
        ...
        realData.destinations.forEach((dest, idx) => {
          ...
          contextPrompt += `${idx + 1}. ${
            dest.destination
          }: ${currencySymbol}${roundedPrice} (cheapest option)\n`;
    ...
    if (tripAdvisorData) {
      ...
      if (tripAdvisorData.location) {
        contextPrompt += `\nLocation: ${
          tripAdvisorData.location.name
        } (geoId: ${
          tripAdvisorData.location.geoId ||
          tripAdvisorData.location.location_id ||
          "N/A"
        })\n`;
      }
      if (
        Array.isArray(tripAdvisorData.attractions) &&
        tripAdvisorData.attractions.length > 0
      ) {
        contextPrompt += `\nTop Attractions:\n`;
        tripAdvisorData.attractions.slice(0, 5).forEach((a, i) => {
          contextPrompt += `- ${a.name || "Attraction"} | Rating: ${
            a.rating || "N/A"
          } | Reviews: ${a.review_count || a.reviewCount || 0}${
            a.price_level ? ` | Price: ${a.price_level}` : ""
          }${a.category ? ` | Category: ${a.category}` : ""}\n`;
        });
      }
      if (
        Array.isArray(tripAdvisorData.restaurants) &&
        tripAdvisorData.restaurants.length > 0
      ) {
        contextPrompt += `\nPopular Restaurants:\n`;
        tripAdvisorData.restaurants.slice(0, 5).forEach((r, i) => {
          const cuisine = Array.isArray(r.cuisine)
            ? r.cuisine.join(", ")
            : r.cuisine || "";
          contextPrompt += `- ${r.name || "Restaurant"} | Rating: ${
            r.rating || "N/A"
          } | Reviews: ${r.review_count || r.reviewCount || 0}${
    ...
    if (realData && realData.type === "multi_destination_comparison") {
      ...
      console.log(
        `💰 Multi-destination comparison currency: ${comparisonCurrency} → Symbol: ${comparisonSymbol}`
      );
      ...
      contextPrompt += `1. User asked about ${
        realData.destinations.length
      ...
      const sortedDests = [...realData.destinations].sort(
      ...
      sortedDests.forEach((dest, idx) => {
        const label =
          idx === 0
            ? "CHEAPEST: "
            : idx === sortedDests.length - 1
            ? "MOST EXPENSIVE: "
            : "";
        const roundedPrice = Math.round(parseFloat(dest.cheapestPrice) || 0);
        contextPrompt += `${idx + 1}. ${label}${
          dest.destination
        } - ${comparisonSymbol}${roundedPrice}\n`;
      });

      contextPrompt += `\nBest Deal: ${
        sortedDests[0].destination
      } is the cheapest at ${comparisonSymbol}${Math.round(
        sortedDests[0].cheapestPrice
      )}. Here are the best options:\n`;
      if (sortedDests[0].flightData.results.length > 0) {
        const topFlight = sortedDests[0].flightData.results[0];
        contextPrompt += `- ${topFlight.airline || "N/A"}, ${
          topFlight.stops === 0 ? "Direct" : topFlight.stops + " stop"
        }, ${topFlight.duration || "N/A"}, Departs ${
          topFlight.departureTime || "TBD"
        }\n`;
      }
      ...
      realData &&
      realData.type === "multi_destination_hotel_comparison"
    ) {
      ...
      if (
        ...
        if (prefs.flightPreferences) {
          ...
          if (fp.preferredAirlines && fp.preferredAirlines.length > 0) {
            prefParts.push(
              `preferred airlines: ${fp.preferredAirlines
                .slice(0, 2)
                .join(", ")}`
            );
          }
          ...
          if (prefParts.length > 0) {
            prefMessage = `Based on your profile preferences (${prefParts.join(
              ", "
            )}), here are the best matching flight options`;
          }
        }
        ...
        contextPrompt += `   - Price: [ROUNDED_PRICE] ${
          realData.currency || "USD"
        }\n`;
        ...
        contextPrompt += `5. CRITICAL: Use currency ${
          realData.currency || "USD"
        }, NOT USD\n`;
        ...
        contextPrompt += `   [GOOGLE_FLIGHTS_BUTTON]${this.buildGoogleFlightsUrlSync(
          realData.request
        )}[/GOOGLE_FLIGHTS_BUTTON]\n\n`;
        ...
        if (
          queryIntent.multiDestination &&
          queryIntent.extractedInfo.destinations &&
          queryIntent.extractedInfo.destinations.length > 1
        ) {
          const origin =
            queryIntent.extractedInfo.origin || userPreferences.homeCity || "";
          ...
          const isCountryClarification =
            realData && realData.type === "country_clarification_needed";
          ...
          if (isCountryClarification) {
            const countryName =
              realData.country.charAt(0).toUpperCase() +
              realData.country.slice(1);
          ...
          // Priority 2: Check if missing origin or destination
            ...
            contextPrompt += `RESPOND EXACTLY LIKE THIS:\n`;
            contextPrompt += `"Perfect! I'll search flights from ${
              queryIntent.extractedInfo?.origin
            } to ${queryIntent.extractedInfo?.destination} for ${
              queryIntent.extractedInfo?.departureDate
            }${
              queryIntent.extractedInfo?.returnDate
                ? ` returning ${queryIntent.extractedInfo.returnDate}`
                : ""
            }.\n\n`;
            ...
            if (origin && destination) {
              ...
              if (typeof passengers === "object" && passengers.adults) {
                googleFlightsUrl += `&adults=${passengers.adults}`;
                if (passengers.children > 0)
                  googleFlightsUrl += `&children=${passengers.children}`;
                if (passengers.infants > 0)
                  googleFlightsUrl += `&infants=${passengers.infants}`;
              } else if (passengerCount > 1) {
                googleFlightsUrl += `&adults=${passengerCount}`;
              }
      ...
      if (
        ...
        if (!queryIntent.extractedInfo?.destination) {
          ...
          !queryIntent.extractedInfo?.checkIn ||
          !queryIntent.extractedInfo?.checkOut
        ) {
          ...
          contextPrompt += `For accommodations in ${
            queryIntent.extractedInfo?.destination || "your destination"
          }."\n\n`;
        }
      }
      ...
      console.log(
        "   🗺️ TRIP PLANNING MODE ACTIVATED - Applying formatting instructions"
      );
      ...
      const departureDate =
        queryIntent.extractedInfo?.departureDate ||
        userPreferences.currentTripStartDate;
      let seasonalInfo = "";

      if (destination && departureDate) {
        const { season, month, monthName } =
          this.getSeasonAndMonth(departureDate);
        const mustDos = this.getSeasonalRecommendations(
          destination,
          season,
          monthName
        );
        ...
        if (mustDos && mustDos.length > 0) {
          ...
          console.log(
            `   🌸 Added ${mustDos.length} seasonal must-dos for ${destination} in ${season}`
          );
        }
      }
      ...
      if (
        ...
        contextPrompt += `CRITICAL: You MUST format your response using proper markdown with headers (###, ####), bold text (**text**), and bullet points (-). This is essential for proper display.\n\n`;
        ...
        contextPrompt += `   - Budget estimates for meals and activities (ONLY include if cost is above $0)\n`;
        contextPrompt += `5. CRITICAL FORMATTING REQUIREMENTS - You MUST use this EXACT markdown structure:\n\n`;
        contextPrompt += `### Detailed Itinerary\n\n`;
        contextPrompt += `#### Day 1: [Descriptive Title]\n\n`;
        contextPrompt += `**Morning:**\n`;
        contextPrompt += `- [Activity with detailed description]\n`;
        contextPrompt += `- [Budget estimate: $XX]\n\n`;
        contextPrompt += `**Afternoon:**\n`;
        contextPrompt += `- [Activity with detailed description]\n`;
        contextPrompt += `- [Budget estimate: $XX]\n\n`;
        contextPrompt += `**Evening:**\n`;
        contextPrompt += `- [Activity with detailed description]\n`;
        contextPrompt += `- [Budget estimate: $XX]\n\n`;
        contextPrompt += `#### Day 2: [Descriptive Title]\n\n`;
        contextPrompt += `[Continue same format for all days...]\n\n`;
        contextPrompt += `### Budget Summary\n\n`;
        contextPrompt += `- **Flights:** $XXX\n`;
        contextPrompt += `- **Hotels:** $XXX per night × X nights = $XXX\n`;
        contextPrompt += `- **Activities:** $XXX\n`;
        contextPrompt += `- **Meals:** $XXX\n`;
        contextPrompt += `- **Total:** $XXX\n\n`;
        contextPrompt += `IMPORTANT: Use exactly this markdown format with ### for main sections, #### for days, **bold** for emphasis, and - for bullet points.\n\n`;
        contextPrompt += `3. Use ALL the data provided above for attractions and restaurants\n`;
        contextPrompt += `4. Make it engaging and descriptive - help them visualize their trip!\n`;
        contextPrompt += `5. DO NOT include flight or hotel recommendations - focus only on the itinerary\n`;
        contextPrompt += `6. For flights and hotels, mention that users can search separately\n`;
        contextPrompt += `7. IMPORTANT: Do NOT show budget estimates of $0 - only show costs when there's an actual expense\n\n`;
      } else {
        ...
        const hasDuration =
          queryIntent.extractedInfo?.duration ||
          queryIntent.extractedInfo?.departureDate;
        ...
        if (!hasDestination) {
          ...
          contextPrompt += `4. CRITICAL FORMATTING REQUIREMENTS - You MUST use this EXACT markdown structure:\n\n`;
          contextPrompt += `### Detailed Itinerary\n\n`;
          contextPrompt += `#### Day 1: [Descriptive Title]\n\n`;
          contextPrompt += `**Morning:**\n`;
          contextPrompt += `- [Activity with detailed description and location]\n`;
          contextPrompt += `- [Budget estimate: $XX]\n\n`;
          contextPrompt += `**Afternoon:**\n`;
          contextPrompt += `- [Activity with detailed description and location]\n`;
          contextPrompt += `- [Budget estimate: $XX]\n\n`;
          ...
          contextPrompt += `[Continue same format for all days...]\n\n`;
          contextPrompt += `### Budget Summary\n\n`;
          contextPrompt += `- **Flights:** Estimate $XXX for 2 travelers (check Google Flights link below)\n`;
          contextPrompt += `- **Hotels:** Estimate $XXX for ${
            queryIntent.extractedInfo?.duration?.value || 4
          } nights\n`;
          contextPrompt += `- **Activities & Meals:** Estimate $XXX total\n`;
          contextPrompt += `- **Total Estimated Cost:** $XXX (within your $${
            userPreferences.budget || 2000
          } budget)\n\n`;
          contextPrompt += `### Practical Travel Tips\n\n`;
          contextPrompt += `- **Transportation:** [Tips for getting around]\n`;
          contextPrompt += `- **Local Customs:** [Cultural etiquette]\n`;
          contextPrompt += `- **Best Times to Visit:** [Optimal timing for attractions]\n`;
          contextPrompt += `- **Money-Saving Tips:** [Budget-friendly advice]\n\n`;
          contextPrompt += `- **Budget Estimates:** [Daily spending guide]\n`;
          contextPrompt += `- **Best Time to Visit:** [Seasonal recommendations]\n\n`;
          contextPrompt += `IMPORTANT: Use exactly this markdown format with ### for main sections, #### for days, **bold** for emphasis, and - for bullet points.\n\n`;
          contextPrompt += `5. DO NOT include flight or hotel recommendations - focus only on the itinerary\n`;
          contextPrompt += `6. For flights and hotels, mention that users can search separately\n`;
          contextPrompt += `7. Make it engaging and descriptive - help them visualize their trip!\n`;
          contextPrompt += `8. IMPORTANT: Do NOT show budget estimates of $0 - only show costs when there's an actual expense\n\n`;
        }
      }
      ...
      if (
        ...
        if (userPreferences.budget || userPreferences.budgetRange) {
          contextPrompt += `- Budget: ${
            userPreferences.budget || userPreferences.budgetRange
          }\n`;
        }
        if (
          userPreferences.preferredDestinations &&
          userPreferences.preferredDestinations.length > 0
        ) {
        ...
        console.log(
          `   🎯 Personalizing recommendations based on interests:`,
          userPreferences.interests
        );
      }
      ...
      if (
        queryIntent.extractedInfo?.queryType === "rules" ||
  ...
  async callBedrock(systemPrompt, conversationHistory, currentQuery) {
    try {
      ...
      if (
        ...
        recentHistory[0].assistant &&
        !recentHistory[0].user
      ) {
        startIndex = 1;
      }
      ...
      if (messages.length === 0 || messages[0].role !== "user") {
        console.error(
          "   ⚠️ Invalid conversation structure, resetting to current query only"
        );
        messages.length = 0;
        messages.push({
          role: "user",
      ...
      if (systemPrompt.includes("MULTI-DESTINATION")) {
        const currencyMatch = systemPrompt.match(
          /currency symbol: (.+?) \(NOT/
        );
        ...
        if (currencyMatch) {
          console.log(
            `   💰 Instructing Nova Pro to use currency: ${currencyMatch[1]}`
          );
        }
        if (destCountMatch && destListMatch) {
          console.log(
            `   🌍 Instructing Nova Pro about ${destCountMatch[1]} destinations: ${destListMatch[1]}`
          );
        }
      }
      ...
      if (responseText.includes("google.com/travel/flights")) {
        const linkCount = (
          responseText.match(/google\.com\/travel\/flights/g) || []
        ).length;
        ...
        if (searchIndex !== -1) {
          const linkSection = responseText.substring(
            searchIndex,
            Math.min(searchIndex + 500, responseText.length)
          );
          console.log(`   📋 Link section:\n${linkSection}`);
        } else {
          console.log(
            `   ⚠️  "Search more options" text not found in response`
          );
          // Try to find where the links are
          const firstLinkIndex = responseText.indexOf(
        ...
        if (lastClose !== -1) {
          ...
          const after = responseText.substring(
            lastClose + "[/GOOGLE_FLIGHTS_BUTTON]".length
          );
          console.log(
            `   🔍 DEBUG: Text after final button marker (${after.length} chars):`
          );
          console.log(
            `   📋 Content: "${after.substring(0, 300).replace(/\n/g, "\\n")}"`
          );

          // If anything meaningful exists after the final marker, remove it.
          if (after.trim().length > 0) {
            console.log(
              `   ☢️ NUCLEAR CLEANUP: Removing ${
                after.trim().length
              } characters after final button marker`
            );
          } else {
            console.log(
              "   ✅ No content found after final button marker (already clean)"
            );
          }

          // Preserve the full block up to the last closing marker
          responseText = responseText
            .substring(0, lastClose + "[/GOOGLE_FLIGHTS_BUTTON]".length)
            .trim();
          console.log(
            "   ✅ Response now ends at final button marker (preserved all markers)"
          );
        }

        // Also check for old format (plain google.com/travel/flights URL without marker)
        const googleFlightsIndex = responseText.lastIndexOf(
          "google.com/travel/flights"
        );
        if (googleFlightsIndex !== -1 && lastClose === -1) {
          console.log(
            "   🔍 Found plain Google Flights URL (old format), checking for numbers..."
          );
          ...
          console.log(
            `   🔍 DEBUG: Text after Google Flights URL (${afterUrl.length} chars):`
          );
          console.log(
            `   📋 First 300 chars: ${afterUrl
              .substring(0, 300)
              .replace(/\n/g, "\\n")}`
          );
          ...
          lines.forEach((line, index) => {
            ...
            if (/^\d{5,}$/.test(trimmed)) {
              ...
              console.log(
                `   📝 Line ${index}: "${trimmed.substring(0, 50)}" (CONTENT)`
              );
            }
          });
          ...
          if (numberLines.length >= 1) {
            console.log(
              `   🧹 Found ${
                numberLines.length
              } standalone number lines: ${numberLines.join(", ")}`
            );
            ...
            responseText =
              responseText.substring(0, urlEnd) +
              "\n" +
              cleanedLines.join("\n");
            console.log(
              `   ✅ Cleaned response, removed ${numberLines.length} standalone number lines`
            );
          }
        }
      } else {
        console.log(
          `   ⚠️  No Google Flights links found in Nova Pro response!`
        );
      }
      ...
      console.log(
        `   🛡️ FINAL CLEANUP: Checking ${lines.length} total lines for trailing numbers...`
      );
      ...
      for (let i = lines.length - 1; i >= 0; i--) {
        ...
        if (
          line.includes("[/GOOGLE_FLIGHTS_BUTTON]") ||
          line.includes("google.com/travel/flights")
        ) {
          lastButtonIdx = i;
          break;
        }
      ...
      if (lastButtonIdx !== -1 && lastButtonIdx < lines.length - 1) {
        ...
        console.log(
          `   ✅ FINAL CLEANUP: Removed trailing numbers after Google Flights button (cutoff at line ${cutoffIdx})`
        );
      } else {
        console.log(
          `   ✅ FINAL CLEANUP: No trailing numbers found after Google Flights button`
        );
      }
      ...
      if (cutoffIndex < lines.length) {
        ...
        console.log(
          `   🛡️ FINAL CLEANUP: Removing ${removedCount} trailing lines (empty or standalone numbers with decimals)`
        );
  ...
  async loadConversationHistory(sessionId, userId = "system") {
    try {
      console.log(
        `🔍 Loading conversation history for session: ${sessionId}, user: ${userId}`
      );
      ...
      if (process.env.CHATS_TABLE) {
        ...
        const chatSession = await chatModel.getChatSession({
          user_id: userId,
          _id: sessionId,
        });
        ...
        if (chatSession && chatSession.messages) {
          console.log(
            `   ✅ Found ${chatSession.messages.length} messages for session ${sessionId}`
          );
          console.log(
          ...
          console.log(
            `   📝 Converted to ${conversationTurns.length} conversation turns`
          );
          return conversationTurns;
        } else {
          console.log(
            `   ⚠️ No chat session found for session ${sessionId}, user ${userId}`
          );
        }
      }
      ...
      console.log(
        `   📝 Using in-memory history: ${memoryHistory.length} messages`
      );
  ...
  async loadUserPreferences(userId) {
    try {
      ...
      try {
        ...
        console.log(
          "   ⚠️ Could not load user profile, using session preferences only"
        );
      }
  ...
  async clearConversation(sessionId) {
    try {
      ...
      if (process.env.CHATS_TABLE) {
        const chatModel = require("../models/chatModel");
        try {
          await chatModel.deleteChatSession({
            user_id: "system",
            _id: sessionId,
          });
        } catch (err) {
      ...
      console.error(
        "Error clearing conversation for session:",
        sessionId,
        error
      );
      return false;
   ...
   */
  async updatePreferencesFromConversation(
    userId,
    userQuery,
    aiResponse,
    currentPreferences
  ...
  getSeasonalRecommendations(destination, season, monthName) {
    const recommendations = {
      "Washington DC": {
        Spring: [
          "Visit the National Cherry Blossom Festival (late March - early April)",
          "Walk through the Tidal Basin when cherry trees are in full bloom",
          "Explore monuments and memorials with perfect weather",
        ],
        Fall: [
          "Walk the National Mall with stunning fall foliage",
          "Visit monuments and memorials during cooler, comfortable weather",
          "Explore Georgetown's historic streets with autumn colors",
          "Rock Creek Park hiking with fall leaves",
        ],
        Summer: [
          "Evening monument tour to avoid daytime heat",
          "Free concerts at the National Mall",
          "Smithsonian museums for air-conditioned exploration",
        ],
        Winter: [
          "National Christmas Tree and holiday decorations",
          "Ice skating at the National Gallery of Art Sculpture Garden",
          "Museum hopping (Smithsonian museums are free!)",
        ],
      },
      "New York": {
        Spring: [
          "Central Park blooms and outdoor concerts",
          "Brooklyn Botanic Garden Cherry Blossom Festival",
          "Outdoor dining in Greenwich Village",
        ],
        Fall: [
          "Fall foliage in Central Park and Prospect Park",
          "Thanksgiving Day Parade (late November)",
          "Rockefeller Center holiday season begins",
        ...
        Winter: [
          ...
          "Times Square New Year's Eve (if in late December)",
          "Indoor museums and Broadway shows",
        ],
      },
      Chicago: {
        Spring: [
          "Millennium Park spring events",
          "Chicago River turns green for St. Patrick's Day (March 17)",
          "Architecture boat tours begin",
        ],
        Fall: [
          "Chicago Marathon (October)",
          "Fall colors along the Lakefront Trail",
          "Magnificent Mile with autumn weather",
        ],
        Summer: [
          "Navy Pier fireworks (Wednesdays and Saturdays)",
          "Chicago beaches and lakefront activities",
          "Millennium Park concerts and events",
          "Outdoor festivals almost every weekend",
        ],
        Winter: [
          "Christkindlmarket (German Christmas market)",
          "Ice skating at Millennium Park",
          "Museum Campus indoor exploration",
        ],
      },
      Tokyo: {
        Spring: [
          "Cherry blossom viewing (Hanami) in Ueno Park and Chidorigafuchi",
          "Sumida River cherry blossom cruise",
          "Spring festivals at temples and shrines",
        ],
        Fall: [
          "Autumn foliage viewing at Meiji Shrine and Rikugien Gardens",
        ...
        Summer: [
          ...
          "Evening exploration to avoid daytime heat",
        ],
        Winter: [
          "Winter illuminations citywide (November-February)",
          "New Year's temple visits (Hatsumode)",
          "Indoor attractions and ramen restaurants",
        ],
      },
      Paris: {
        Spring: [
          "Gardens blooming at Tuileries and Luxembourg",
          "Seine river cruises with pleasant weather",
          "Outdoor cafés reopening",
        ],
        Fall: [
          "Fall colors in Luxembourg Gardens and Bois de Boulogne",
          "Wine harvest season in nearby regions",
          "Fashion Week (late September)",
          "Montmartre with autumn vibes",
        ],
        Summer: [
        ...
        Winter: [
          "Christmas markets on Champs-Élysées",
      ...
      Barcelona: {
        Spring: [
          "Sant Jordi festival (April 23) - books and roses",
          "Beach season begins",
          "Park Güell and outdoor Gaudí sites",
        ],
        Fall: [
          "La Mercè festival (September)",
          "Beach still warm enough for swimming (September)",
          "Wine harvest in nearby Penedès region",
        ...
        Winter: [
          ...
          "Indoor Picasso and modern art museums",
        ],
      },
      London: {
        Spring: [
          "Kew Gardens in bloom",
          "Thames river walks with spring weather",
          "Outdoor markets reopening",
        ],
        Fall: [
          "Hyde Park with autumn colors",
          "Bonfire Night (November 5)",
          "Covent Garden fall atmosphere",
        ...
        Summer: [
          ...
          "Outdoor theater and cinema",
          "Notting Hill Carnival (August Bank Holiday)",
        ],
        Winter: [
          "Winter Wonderland in Hyde Park",
          "Christmas lights on Oxford Street and Regent Street",
          "New Year's Eve fireworks on the Thames",
          "Cozy pubs and afternoon tea",
        ],
      },
      Dubai: {
        Spring: [
          "Perfect beach weather",
          "Desert safaris with pleasant temperatures",
          "Outdoor markets and souks",
        ],
        Fall: [
          "Start of pleasant outdoor weather",
          "Dubai Shopping Festival begins (late fall)",
          "Desert activities become comfortable",
        ],
        Summer: [
          "Dubai Summer Surprises (shopping festival)",
          "Indoor mall exploration with A/C",
          "Indoor ski resort and attractions",
          "Late evening beach visits",
        ],
        Winter: [
          "Dubai Shopping Festival (December-January)",
          "Perfect weather for all outdoor activities",
          "Desert camping and safaris",
      ...
      Rome: {
        Spring: [
          "Easter celebrations at Vatican City",
          "Perfect weather for Colosseum and Roman Forum",
          "Rome's Birthday celebration (April 21)",
          "Outdoor dining in Trastevere",
        ],
        Fall: [
          "Wine harvest season in nearby regions",
          "Comfortable walking weather for ancient sites",
          "Fall food festivals",
          "Fewer tourists at major attractions",
        ],
        Summer: [
          "Evening strolls around Trevi Fountain",
          "Outdoor concerts and festivals",
          "Late opening hours at major sites",
          "Gelato tours (peak season!)",
        ],
        Winter: [
          "Christmas markets and nativity scenes",
          "New Year's at the Colosseum area",
          "Indoor museums and churches",
          "Fewer crowds at Vatican",
        ],
      },
      Amsterdam: {
        Spring: [
          "Tulip season (mid-March to May)",
          "Keukenhof Gardens in full bloom",
          "King's Day celebration (April 27)",
          "Canal boat tours with spring weather",
        ],
        Fall: [
          "Amsterdam Dance Event (October)",
          "Fall colors along canals",
          "Museum season with fewer crowds",
          "Cozy brown café culture",
        ],
        Summer: [
          "Outdoor festivals and concerts",
          "Canal swimming spots",
          "Vondelpark picnics",
          "Open-air cinema",
          "Pride Amsterdam (early August)",
        ],
        Winter: [
          "Ice skating on canals (if frozen)",
          "Amsterdam Light Festival (December-January)",
          "Christmas markets",
          "Cozy museums and rijsttafel dinners",
        ],
      },
      Singapore: {
        Spring: [
          "Perfect weather before monsoon",
          "Gardens by the Bay in bloom",
          "Food festivals",
        ...
        Summer: [
          ...
          "Indoor attractions and malls",
          "Evening Marina Bay activities",
        ],
        Winter: [
      ...
      Sydney: {
        Spring: [
          "Jacaranda trees blooming (October-November)",
          "Sculpture by the Sea (October-November)",
          "Comfortable beach weather begins",
          "Coastal walks with perfect weather",
        ],
        Fall: [
          "Sydney Film Festival",
          "Perfect beach weather continues",
          "Vivid Sydney (May-June) - light festival",
          "Autumn colors in gardens",
        ],
        Summer: [
          "Bondi Beach season",
          "Sydney New Year's Eve fireworks (world-famous!)",
          "Outdoor concerts and festivals",
      ...
      Bali: {
        Spring: [
          "Perfect weather before rainy season",
          "Nyepi (Balinese New Year) - Day of Silence",
          "Beach activities and surfing",
          "Rice terrace tours in ideal weather",
        ],
        Fall: [
          "Start of dry season",
        ...
        Summer: [
          "Peak dry season - perfect weather",
          "Ubud Food Festival (April)",
          "Best surfing conditions",
          "Temple festivals throughout",
        ],
        Winter: [
          "Some rain but still warm",
          "Fewer tourists in December",
          "New Year celebrations",
          "Indoor cultural activities and spa",
        ],
      },
      Mumbai: {
        Spring: [
          "Perfect weather after winter",
          "Beach activities at Marine Drive",
          "Outdoor markets and street food tours",
          "Heritage walks in pleasant weather",
        ],
        Fall: [
          "Ganesh Chaturthi festival (August-September)",
          "Navratri celebrations (September-October)",
          "Post-monsoon pleasant weather",
          "Diwali festivities (October-November)",
        ],
        Summer: [
          "Indoor attractions and museums",
          "Monsoon season begins (June) - experience romantic rains",
          "Mall exploration with A/C",
          "Evening Marine Drive walks",
        ],
        Winter: [
          "Perfect weather for everything!",
          "Kala Ghoda Arts Festival (February)",
          "Beach and outdoor dining",
      ...
      Bangkok: {
        Spring: [
          ...
          "Perfect weather before hot season",
          "Temple visits comfortable",
          "Night markets in pleasant weather",
        ],
        Fall: [
          "End of rainy season",
        ...
        Summer: [
          "Indoor temple exploration",
          "Shopping mall culture",
          "Rooftop bars for evening views",
          "Water markets in early morning",
        ],
        Winter: [
      ...
      Athens: {
        Spring: [
          "Easter celebrations (Greek Orthodox)",
          "Perfect weather for Acropolis visits",
          "Wildflowers blooming",
          "Outdoor tavernas opening",
        ],
        Fall: [
          "Athens Epidaurus Festival continues",
          "Comfortable weather for ancient sites",
          "Wine harvest season",
        ...
        Summer: [
          ...
          "Evening visits to ancient sites",
        ],
        Winter: [
          "Fewer crowds at major sites",
          "Christmas decorations in Syntagma Square",
          "Indoor museums and archaeological sites",
          "Traditional tavernas cozy atmosphere",
        ],
      },
      Madrid: {
        Spring: [
          "San Isidro Festival (May)",
          "Perfect weather for Retiro Park",
          "Outdoor terrazas (terraces) opening",
          "Art museum season begins",
        ],
        Fall: [
          "Autumn art exhibitions at Prado and Reina Sofia",
          "Perfect walking weather",
          "Tapas season in full swing",
          "Fewer tourists than summer",
        ],
        Summer: [
          "Veranos de la Villa cultural festival",
          "Late-night dining culture peaks",
          "Rooftop bars with city views",
          "Day trips to escape heat",
        ],
        Winter: [
          "Christmas lights on Gran Via",
          "Three Kings Parade (January 5)",
          "Indoor museums (Prado, Reina Sofia)",
          "Traditional hot chocolate and churros",
        ],
      },
    };
    ...
    const partialMatch = Object.keys(recommendations).find(
      (key) =>
    ...
    const genericAdvice = {
      Spring: [
        "Enjoy outdoor sightseeing with pleasant weather",
        "Visit gardens and parks in bloom",
      ],
      Summer: [
        "Early morning or evening activities to avoid heat",
        "Enjoy longer daylight hours",
      ],
      Fall: [
        "Perfect weather for walking tours",
        "Enjoy fall colors if applicable",
      ],
      Winter: [
        "Bundle up for outdoor sightseeing",
        "Explore indoor attractions and museums",
      ],
    };
  ...
  buildGoogleFlightsUrlSync(searchRequest) {
    ...
    if (departureDate && departureDate.includes("2023")) {
      ...
      console.log(
        `⚠️  Fixed departure date: ${searchRequest.departureDate} → ${departureDate}`
      );
    }
    if (returnDate && returnDate.includes("2023")) {
      returnDate = returnDate.replace("2023", "2025");
      console.log(
        `⚠️  Fixed return date: ${searchRequest.returnDate} → ${returnDate}`
      );
    }
  ...
  removeEmojis(text) {
    ...
    return text.replace(
      /[\u{1F600}-\u{1F64F}]|[\u{1F300}-\u{1F5FF}]|[\u{1F680}-\u{1F6FF}]|[\u{1F1E0}-\u{1F1FF}]|[\u{2600}-\u{26FF}]|[\u{2700}-\u{27BF}]|[\u{1F900}-\u{1F9FF}]|[\u{1F018}-\u{1F270}]|[\u{238C}-\u{2454}]|[\u{20D0}-\u{20FF}]|[\u{FE00}-\u{FE0F}]|[\u{E0020}-\u{E007F}]|[\u{1F004}]|[\u{1F0CF}]|[\u{1F170}-\u{1F251}]/gu,
      ""
    );
  }
}
